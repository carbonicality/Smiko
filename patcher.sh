...
function patch_shim_root() {
	...
	cp "${SCRIPT_DIR}/util/shim_gui.sh" "$root/usr/sbin/factory_install.sh"
	...
	if ! fbool legacy; then
		mkdir -p "$root/usr/share/smiko/assets"
		cp -rf "${SCRIPT_DIR}/assets" "$root/usr/share/smiko/assets"
		chmod +r "$root/usr/share/smiko/assets" --recursive
	else
		# Assets shouldn't exist on legacy systems.
		if [ -d "$root/usr/share/smiko/assets" ]; then 
			rm -rf "$root/usr/share/smiko/assets"
		fi
	fi
	...
	local firmware_path=
	for bin in "$SCRIPT_DIR"/lib/stock/*; do
...
function patch_sh1mmer() {
	...
	info "Copying scripts to image."
	cp "${SCRIPT_DIR}/util/shim_gui.sh" "$sh1mmer/root/noarch/payloads/smiko.sh"
	...
	if ! fbool legacy; then
		mkdir -p "$sh1mmer/root/noarch/usr/share/smiko/assets"
		cp -rf "${SCRIPT_DIR}/assets" "$sh1mmer/root/noarch/usr/share/smiko/assets"
		chmod +r "$sh1mmer/root/noarch/usr/share/smiko/assets" -R --recursive
	fi
...
function loadopts() {
	...
	DEFINE_boolean legacy "$FLAGS_FALSE" "Build Smiko for a pre-frecon image (TUI base instead of a GUI)" "l"
